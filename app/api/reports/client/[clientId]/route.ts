...
export async function GET(
  ...
  try {
    ...
    const supplierIds = topProviders
      .map(p => p.supplierId)
    ...
    const topProvidersWithDetails = topProviders.map(tp => {
      const supplier = suppliers.find(s => s.id === tp.supplierId)
      return {
        supplierId: tp.supplierId,
        supplierName: supplier?.name || 'Desconocido',
        category: supplier?.categories[0]?.category,
        rating: supplier?.score?.overallScore || 0,
    ...
    const mostCompetitiveSupplier = topProvidersWithDetails[0]

    const insight = {
      monthlySavings: monthSavingsPercentage > 0 
        ? `${monthSavingsPercentage.toFixed(1)}%`
        : 'N/A',
      topSupplier: mostCompetitiveSupplier?.supplierName || 'N/A',
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        // Report 3: Top Suppliers
        topSuppliers: topProvidersWithDetails,
