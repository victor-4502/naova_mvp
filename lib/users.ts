// Sistema de gestión de usuarios - Compatible con desarrollo y producción
// En desarrollo: usa archivos (server-users.json)
// En producción: usa Prisma (base de datos)
...
// Detectar si estamos en producción (con base de datos)
const USE_PRISMA = process.env.DATABASE_URL && process.env.NODE_ENV === 'production'

// ==================== FUNCIONES PARA PRISMA (PRODUCCIÓN) ====================
...
// ==================== API PÚBLICA (DETECTA AUTOMÁTICAMENTE) ====================
...
export function getUsersByRole(role: 'ADMIN' | 'CLIENT'): User[] | Promise<User[]> {
  if (USE_PRISMA) {
    return prisma.user.findMany({
      where: { role: role === 'ADMIN' ? 'admin' : 'client' },
      orderBy: { createdAt: 'desc' },
